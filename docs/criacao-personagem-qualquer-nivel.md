...
**Legenda:** ✅ concluído · ⏭️ deferido · ⬜ pendente
...
- ✅ Constantes ou tabela: ouro inicial por nível (PHB 2024) — `starting_gold_tiers` + `domain/equipment/starting-gold.ts`
- ✅ View ou parser: elegibilidade de feat por prerequisito — parser básico em `feats.ts`
- ✅ Coluna `subclasses.unlock_level` (default 3) + constante `SUBCLASS_UNLOCK_LEVEL`
...
- ⏭️ `SpellProgressionSnapshot` — cantrips / preparadas / grimório @ nível N *(deferido: modelo flat)*
- ⏭️ `SpellSelectionBucket` — escolhas por nível desbloqueado *(deferido: escolha tudo de uma vez)*
...
- ✅ `buildSpellsRpcPayload` — magias acumuladas de todos os níveis *(modelo flat cobre o total @ N)*
...
- ✅ Truques @ nível N
- ✅ Preparadas @ nível N
- ✅ Slots pact (automático via sync — exibidos na ficha)

#### Paladin (half)

- ✅ Sem truques
- ✅ Preparadas @ nível N (`paladin-prepared`)
- ✅ Slots half (automático via sync — exibidos na ficha)
...
- ✅ Nível 2+: referência PHB de ouro na UI (payload de ouro = v2)
- ✅ UI toggle equipamento vs ouro (v2)
- ✅ Payload inventário ou campo `starting_gold` (migration `000044`)
...
- ✅ `StepDetails.tsx` — resumo com nível, subclasse, feats de progressão e ouro ref.
- ✅ `CharacterBuilderWizard.tsx` — details key + reset por nível
- ✅ `builder-detail-content.tsx` — features até nível selecionado
...
- ✅ Slots de magia corretos @ nível N (`v_class_spell_slots` + sync + ficha)
- ✅ Rage / Second Wind / Channel Divinity / Ki — `sync_character_resources` *(testes `expected-resources`)*
- ✅ Bônus de proficiência @ nível total *(RPC + badge na ficha)*
...
- ✅ Unit: Wizard spellbook total @ 1, 5, 10, 20
...
- ✅ Itens mágicos por nível (DMG tables simplificadas)
...
- ✅ Equipamento nível 2+: ouro vs pacote background *(toggle v2 + `starting_gold_gp`)*
- ✅ Wizard spellbook: migration vs fórmula hardcoded temporária *(fórmula em `spell-progression.ts`)*
- ✅ Magias: UI por nível desbloqueado vs “escolha tudo de uma vez” *(escolha tudo de uma vez)*
- ✅ Subclasse no passo Classe vs aba Escolhas *(passo Classe)*
...
*Última atualização: jun/2026 — Fases 1–4 implementadas (multiclasse, level-up, loja, import/export).*
